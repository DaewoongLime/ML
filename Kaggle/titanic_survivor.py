...
test_df.drop(columns=['PassengerId', 'Ticket', 'Cabin', 'Name'], inplace=True)
...
encoded_embarked = pd.get_dummies(test_df['Embarked'], prefix='Embarked')
for col in ['Embarked_C', 'Embarked_Q', 'Embarked_S']:
    if col in encoded_embarked.columns and encoded_embarked[col].dtype == 'bool':
        encoded_embarked[col] = encoded_embarked[col].astype(int)

test_df = pd.concat([test_df, encoded_embarked], axis=1)
...
submission_df = pd.DataFrame({
    'PassengerId': test_passenger_ids, # 미리 저장해둔 PassengerId 사용
    'Survived': predictions_binary.flatten() # 예측 결과를 1차원 배열로 평탄화
})
